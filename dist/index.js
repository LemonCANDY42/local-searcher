...
// src/rerank-versions.ts
var SUPPORTED_RERANK_VERSIONS = ["v1.0", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v2.0"];
var DEFAULT_RERANK_VERSION = "v1.4";
function isSupportedRerankVersion(value) {
  return typeof value === "string" && SUPPORTED_RERANK_VERSIONS.includes(value);
}
...
var DEFAULTS = {
  ...
  maxLinks: 24,
  fetchTimeoutMs: 2e4,
  browserTimeoutMs: 45e3,
  rerankEnabled: true,
  defaultMode: "auto",
  defaultRerankVersion: DEFAULT_RERANK_VERSION
};
var MAX_QUERY_CATEGORIES = 2;
var RETRIEVAL_FIRST_RERANK_VERSIONS = /* @__PURE__ */ new Set(["v1.4", "v1.5", "v2.0"]);
var PLANNER_CANDIDATE_RERANK_VERSIONS = /* @__PURE__ */ new Set(["v1.5", "v2.0"]);
var DOC_HOST_HINTS = [
  "platform.openai.com",
  "docs.openai.com",
  "readthedocs.io",
...
var DOC_PATH_HINTS = ["/docs", "/doc", "/guide", "/guides", "/api", "/reference", "/manual", "/readme"];
var MODEL_HOST_HINTS = ["huggingface.co", "hf-mirror.com", "modelscope.cn", "modelscope.com"];
var PACKAGE_HOST_HINTS = ["pypi.org", "npmjs.com", "github.com"];
var DEMOTED_HOSTS = ["hub.docker.com", "docker.com", "dockerhub.com"];
var SEO_HOST_HINTS = ["stackoverflow.ai", "libhunt.com", "sourceforge.net"];
var LOW_SIGNAL_QUERY_TOKENS = /* @__PURE__ */ new Set([
  "a",
  "an",
  "and",
  "api",
...
var AGENT_TASK_MODES = ["lookup", "extract", "compare"];
var AGENT_TARGET_KINDS = ["general", "official-doc", "release-artifact", "whats-new", "model-choice", "product-eval"];
var AGENT_SOURCE_TRUST_LEVELS = ["balanced", "official-first"];
var ARTIFACT_QUERY_TOKENS = /* @__PURE__ */ new Set([
  "release",
  "released",
  "releases",
  "notes",
...
function isAgentTaskMode(value) {
  return typeof value === "string" && AGENT_TASK_MODES.includes(value);
}
function isAgentTargetKind(value) {
  return typeof value === "string" && AGENT_TARGET_KINDS.includes(value);
}
function isAgentSourceTrust(value) {
  return typeof value === "string" && AGENT_SOURCE_TRUST_LEVELS.includes(value);
}
function normalizeAgentSearchContract(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  const raw = value;
  const taskMode = isAgentTaskMode(raw.taskMode) ? raw.taskMode : void 0;
  const targetKind = isAgentTargetKind(raw.targetKind) ? raw.targetKind : void 0;
  const sourceTrust = isAgentSourceTrust(raw.sourceTrust) ? raw.sourceTrust : void 0;
...
function agentContractHasTargetKind(intent, ...kinds) {
  return Boolean(intent.agentContract?.targetKind && kinds.includes(intent.agentContract.targetKind));
}
function agentContractPrefersOfficialSources(intent) {
  return intent.agentContract?.sourceTrust === "official-first";
}
var V12_EMBEDDING_WEIGHT = 0.75;
var V12_HEURISTIC_PRIOR_WEIGHT = 0.25;
var V13_PROFILE_PRESETS = {
...
var DEFAULT_LOCAL_EMBEDDING_MODEL_REF = "hf:ggml-org/embeddinggemma-300m-qat-q8_0-GGUF/embeddinggemma-300m-qat-Q8_0.gguf";
var DEFAULT_LOCAL_EMBEDDING_MODEL_PATH = path.join(os.homedir(), ".cache", "openclaw-memory-models", "embeddinggemma-300m-qat-Q8_0.gguf");
var FALLBACK_SEMANTIC_VECTOR_SIZE = 384;
var localEmbeddingProviderPromise = null;
...
function slugify(input) {
  return input.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "query";
}
...
function stripHtml(html) {
  return decodeHtmlEntities(
    html.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi, " ").replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, " ").replace(/<noscript\b[^>]*>[\s\S]*?<\/noscript>/gi, " ").replace(/<!--[\s\S]*?-->/g, " ").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()
  );
}
function excerpt(text, maxChars = 320) {
...
function ensureWithinLimit(items, limit) {
...
function structuredTokenFragments(token) {
  return uniqueStrings(
    normalizeText(token).split(/[._/:+-]+/g).map((fragment) => fragment.trim()).filter((fragment) => fragment.length >= 2 && !/^\d+$/.test(fragment) && fragment !== token)
  );
}
...
function strictStructuredTokenMatch(text, token) {
  const normalizedToken = normalizeText(token);
  if (!normalizedToken) {
    return false;
  }
...
function textIncludesQueryPhrase(text, query) {
...
function isSourceLikeQueryToken(token) {
...
function contentLikeQueryTokens(tokens) {
  return tokens.filter((token) => !isSourceLikeQueryToken(token));
}
function appendMissingFragments(query, fragments) {
  const normalized = normalizeText(query);
  const missing = fragments.map((fragment) => fragment.trim()).filter(Boolean).filter((fragment) => !normalized.includes(normalizeText(fragment)));
  if (missing.length === 0) {
    return query.trim();
  }
...
var ROMANIZED_CHINESE_SURNAMES = /* @__PURE__ */ new Set([
  "ai",
  "an",
  "bai",
  "bao",
...
var ROMANIZED_CHINESE_COMPOUND_SURNAMES = /* @__PURE__ */ new Set(["ouyang", "sima", "shangguan", "situ", "zhuge"]);
var ROMANIZED_NAME_ORG_HINTS = /* @__PURE__ */ new Set([
  "ai",
  "auto",
  "bank",
  "capital",
...
function looksLikeRomanizedChineseNameToken(token) {
...
function specializeRomanizedChineseNameQuery(input) {
  ...
  if ((ROMANIZED_CHINESE_SURNAMES.has(first) || ROMANIZED_CHINESE_COMPOUND_SURNAMES.has(first)) && looksLikeRomanizedChineseNameToken(parts[0]) && looksLikeRomanizedChineseNameToken(parts[1]) && !ROMANIZED_NAME_ORG_HINTS.has(second)) {
...
function normalizeEnglishSearchToolInput(params) {
...
function isModelHost(host) {
  return MODEL_HOST_HINTS.some((hint) => hostMatches(host, hint));
}
function isPackageHost(host) {
  return PACKAGE_HOST_HINTS.some((hint) => hostMatches(host, hint));
}
function isPackageRegistryHost(host) {
  return hostMatches(host, "pypi.org") || hostMatches(host, "npmjs.com");
}
function looksLikePackagePage(pagePath, title) {
  const normalizedTitle = normalizeText(title);
  return pagePath.includes("/package/") || pagePath.includes("/npm/") || pagePath.includes("/pypi/") || hasAny(normalizedTitle, [" on npm", " on pypi", " npm package", " pypi package"]);
}
function isDemotedHost(host) {
  return DEMOTED_HOSTS.some((hint) => hostMatches(host, hint));
}
function isSeoLikeHost(host) {
  return SEO_HOST_HINTS.some((hint) => hostMatches(host, hint));
}
function brandOfficialSignals(intent, host) {
  const signals = [];
...
function hasDocsHostPrefix(host) {
  return host.startsWith("docs.") || host.startsWith("developer.") || host.startsWith("developers.");
}
function looksLikeOfficialDocs(host, pagePath, title) {
  const normalizedTitle = normalizeText(title);
  return DOC_HOST_HINTS.some((hint) => hostMatches(host, hint)) || hasDocsHostPrefix(host) || DOC_PATH_HINTS.some((hint) => pagePath.includes(hint)) || hasAny(normalizedTitle, ["documentation", "docs", "guide", "reference", "api"]);
}
function clampScore(value) {
  return Number(clamp01(value).toFixed(4));
}
function hasUrlLikeText(input) {
  return /https?:\/\/\S+/i.test(input);
}
function isShallowPath(pagePath) {
  return pagePath === "/" || pagePath.split("/").filter(Boolean).length <= 1;
}
function hasArtifactCue(...parts) {
  ...
  const looksLikeVersionedDocsSnapshot = combined.includes("/download/release/") && (combined.includes("/docs/") || combined.includes(" documentation") || combined.includes("api reference")) && !hasAny(combined, [
    "release notes",
    "changelog",
    "what's new",
    "whats new",
...
function focusEntityTokensV20(intent) {
...
function precisionAnchorTokensV20(intent) {
  return uniqueStrings(
...
function criticalAnchorTokensV20(intent) {
  return precisionAnchorTokensV20(intent).filter((token) => {
    const normalized = normalizeText(token);
    return normalized.length <= 4 || /\d/.test(normalized) || /[._/:+-]/.test(normalized);
  });
}
function artifactAnchorTokensV20(intent) {
  return uniqueStrings(
    salientTokens(intent.tokens).filter((token) => ARTIFACT_QUERY_TOKENS.has(normalizeText(token)))
  );
}
function hasArtifactIntentV20(intent) {
  return artifactAnchorTokensV20(intent).length > 0;
}
function looksLikeVersionedDocsSnapshotV20(result) {
  const pagePath = normalizeText(result.path);
  const title = normalizeText(result.title);
  const hasVersionedDocsPath = pagePath.includes("/download/release/v") || pagePath.includes("/docs/") && /\/v?\d+(?:\.\d+){1,3}\//.test(pagePath);
  const genericDocsSnapshot = hasAny(title, ["about this documentation", "documentation"]) && hasAny(pagePath, ["/documentation.html", "/docs/api/", "/docs/"]);
  return hasVersionedDocsPath && genericDocsSnapshot;
}
function resultLexicalAnchorCoverageV20(result, intent) {
...
function resultCriticalAnchorCoverageV20(result, intent) {
...
function resultArtifactAnchorCoverageV20(result, intent) {
...
function hasAnalysisCommentaryCueV20(result) {
...
function hasTroubleshootingCueV20(result) {
  const combined = normalizeText(`${result.title} ${result.snippet} ${result.path}`);
  return hasAny(combined, [
    "troubleshooting",
    "troubleshoot",
...
var EXPLAINER_QUERY_TOKENS_V20 = /* @__PURE__ */ new Set([
  "what",
  "how",
  "works",
  "work",
...
function resultExplainerAspectCoverageV20(result, intent) {
...
function hasExplainerCueV20(result) {
  const combined = normalizeText(`${result.title} ${result.snippet} ${result.path}`);
  return hasAny(combined, [
    "what is",
    "how it works",
...
function resultEntityCoverageV20(result, intent) {
...
function resultHostCoverageV20(result, intent) {
  const tokens = focusEntityTokensV20(intent);
  if (tokens.length === 0) {
    return 0;
  }
...
function titleTokenCoverage(text, tokens) {
  if (tokens.length === 0) {
    return 0;
  }
  return countTokenMatches(text, tokens) / tokens.length;
}
function isStructuredLookupResultType(resultType) {
  return resultType === "official-docs" || resultType === "package" || resultType === "repo" || resultType === "issue-thread" || resultType === "tutorial";
}
function isRetrievalFirstRerankVersion(version) {
  return RETRIEVAL_FIRST_RERANK_VERSIONS.has(version);
}
function isPlannerCandidateRerankVersion(version) {
  return PLANNER_CANDIDATE_RERANK_VERSIONS.has(version);
}
function buildPlannerOutput(query, intent, requestedCategory) {
  ...
  const contractOfficialLookup = agentContractPrefersOfficialSources(intent) || agentContractHasTargetKind(intent, "official-doc", "release-artifact", "whats-new");
  ...
  const hasExtractionHint = contractExtract || agentContractHasTargetKind(intent, "release-artifact", "whats-new") || hasUrlLikeText(query) || hasAny(normalizedQuery, [
    " extract",
    " extraction",
    " parse",
    " scrape",
  ...
  const exactEntityLikely = hasQuotedEntity || hasOfficialHint || contractOfficialLookup || hasVersionLikePattern || intent.sourceMentions.length > 0 || intent.mode === "official-docs" || intent.mode === "models" || hasErrorLikePattern;
  const solutionIntentLikely = hasHowToHint || hasErrorLikePattern || intent.mode === "github" || hasAny(normalizedQuery, [" repo", " repository", " package", " sdk", " tool", " issue", " plugin", " github", " npm", " pypi"]);
  ...
  let precisionDial = "balanced";
  if (branch === "broad-discovery" || verifySensitive && !exactEntityLikely) {
    precisionDial = "broad";
  } else if (branch === "precision-lookup" || hasQuotedEntity || hasVersionLikePattern || hasOfficialHint && !hasComparisonHint) {
    precisionDial = "precise";
  }
  let expectedNextStep = "fetch";
...
function classifyResultType(result) {
  ...
  if (result.category === "news" || hasAny(host, ["reuters.com", "apnews.com", "bloomberg.com", "ft.com", "nytimes.com", "theverge.com", "techcrunch.com"]) || pagePath.includes("/news/")) {
    why.push("news source/path");
    return { resultType: "news", why };
  }
  if (pagePath.includes("/blog") || pagePath.includes("/post") || pagePath.includes("/article") || hasAny(host, ["medium.com", "substack.com", "dev.to", "hashnode.dev"])) {
    why.push("blog/article path");
    return { resultType: "blog", why };
  }
  if (isSeoLikeHost(host) || hasAny(title, ["alternatives", "top ", "best ", "directory", "list of", "list releases", "release date", "end of life", "\u6392\u540D", "\u5408\u96C6"]) || pagePath.includes("/search") || pagePath.includes("/tag/") || pagePath.includes("/category/")) {
...
function classifyPageRole(result, resultType, why) {
...
function scoreSolutionLikelihood(result, resultType, intent, why) {
  const combined = normalizeText(`${result.title} ${result.snippet} ${result.path}`);
  const baseScores = {
    "official-docs": 0.72,
    repo: 0.94,
...
function scoreEntityMatchStrength(result, intent, why) {
...
function scoreSourceFit(result, resultType, intent, pageRole, why) {
  const host = normalizeText(result.host);
  const pagePath = normalizeText(result.path);
  ...
  const exactPhrase = Boolean(intent.normalizedQuery) && (textIncludesQueryPhrase(result.title, intent.normalizedQuery) || textIncludesQueryPhrase(result.path, intent.normalizedQuery));
  ...
  if (intent.sourceMentions.includes("huggingface") && hostMatches(host, "huggingface.co") || intent.sourceMentions.includes("hf-mirror") && hostMatches(host, "hf-mirror.com") || intent.sourceMentions.includes("modelscope") && (hostMatches(host, "modelscope.cn") || hostMatches(host, "modelscope.com"))) {
    score = Math.max(score, 0.98);
...
function scorePageSpecificity(result, resultType, intent, why) {
  const title = normalizeText(result.title);
  const pagePath = normalizeText(result.path);
  const tokens = (() => {
    const focusTokens = salientTokens(intent.tokens);
  ...
  if (resultType === "official-docs" && (pagePath === "/docs" || pagePath === "/api/docs" || pagePath === "/reference" || pagePath === "/api/reference")) {
    score -= 0.14;
...
function scoreExtractionLikelihood(result, resultType, why) {
  const pagePath = normalizeText(result.path);
...
function scoreDiversityValue(result, resultType, hostCounts, typeCounts) {
...
function plannerAdjustmentForResult(result, diagnostics, intent, planner) {
  const canonicalBoost = diagnostics.resultType === "official-docs" || diagnostics.resultType === "package" || diagnostics.resultType === "repo" ? 0.04 : 0;
  const practicalBoost = diagnostics.resultType === "repo" || diagnostics.resultType === "package" || diagnostics.resultType === "issue-thread" || diagnostics.resultType === "tutorial" ? 0.05 : 0;
  const extractableBoost = diagnostics.resultType === "official-docs" || diagnostics.resultType === "tutorial" || diagnostics.resultType === "blog" || diagnostics.resultType === "news" ? 0.04 : 0;
  const genericPenalty = diagnostics.resultType === "directory" || diagnostics.resultType === "landing" ? -0.06 : diagnostics.resultType === "news" && planner.branch === "solution-hunt" ? -0.05 : 0;
  let adjustment = 0;
  switch (planner.branch) {
    case "broad-discovery": {
      ...
      if (diagnostics.resultType === "repo" && !planner.flags.exactEntityLikely && !planner.queryProfile.hasOfficialHint && !planner.queryProfile.hasComparisonHint && !planner.queryProfile.hasSimilarityHint) {
        adjustment -= 0.04;
      }
...
function shouldApplyV15GuardedLayer(intent, planner) {
  ...
  if (planner.branch === "solution-hunt" && (intent.docsLike || intent.githubLike || intent.modelLike || intent.packageLike || intent.sourceMentions.length > 0)) {
    return true;
  }
  return false;
}
function guardedAdjustmentForResult(result, diagnostics, intent, planner) {
  if (!shouldApplyV15GuardedLayer(intent, planner)) {
    return 0;
  }
  const hostCoverage = resultHostCoverageV20(result, intent);
  let adjustment = diagnostics.sourceFitScore * 0.16 + diagnostics.pageSpecificity * 0.12 + diagnostics.entityMatchStrength * (planner.branch === "precision-lookup" ? 0.08 : 0.04);
  ...
  if (planner.branch === "precision-lookup" && planner.flags.exactEntityLikely && diagnostics.resultType === "official-docs" && diagnostics.entityMatchStrength < 0.08 && diagnostics.pageSpecificity < 0.12) {
    adjustment -= 0.18;
  }
  if (planner.branch === "precision-lookup" && intent.mode === "official-docs" && diagnostics.resultType === "official-docs" && diagnostics.sourceFitScore < 0.8 && diagnostics.entityMatchStrength < 0.36) {
    adjustment -= 0.12;
  }
...
function annotateResultDiagnostics(results, intent, planner, options) {
  ...
  const enriched = typed.map(({ result, diagnostics }) => {
    ...
    return {
      ...
      guardedAdjustment,
      why: options.debug ? diagnostics.why : void 0
    };
  });
  ...
  return enriched.sort(
    (a, b) => (b.score ?? 0) - (a.score ?? 0) || (planner.branch === "precision-lookup" ? (b.entityMatchStrength ?? 0) - (a.entityMatchStrength ?? 0) : planner.branch === "solution-hunt" ? (b.solutionLikelihood ?? 0) - (a.solutionLikelihood ?? 0) : planner.branch === "extract-heavy" ? (b.extractionLikelihood ?? 0) - (a.extractionLikelihood ?? 0) : (b.diversityValue ?? 0) - (a.diversityValue ?? 0)) || (b.heuristicPrior ?? 0) - (a.heuristicPrior ?? 0) || (a.rank ?? 999) - (b.rank ?? 999) || a.originalRank - b.originalRank
  ).map((result, index) => ({
    ...result,
    rank: index + 1
  }));
}
function tokenSetJaccard(left, right) {
...
function selectCandidateSet(results, planner, limit, debug) {
...
function normalizeSearchResults(raw, limit, category, offset = 0) {
  const results = Array.isArray(raw?.results) ? raw.results : [];
  return ensureWithinLimit(results, limit).map((item, index) => {
    const url = typeof item?.url === "string" ? item.url : "";
    return {
      title: typeof item?.title === "string" ? item.title : "",
      url,
      snippet: typeof item?.content === "string" ? item.content : typeof item?.snippet === "string" ? item.snippet : "",
      engine: typeof item?.engine === "string" ? item.engine : void 0,
      category: typeof item?.category === "string" ? item.category : void 0,
      publishedDate: typeof item?.publishedDate === "string" ? item.publishedDate : typeof item?.published === "string" ? item.published : void 0,
...
function detectQueryIntent(query, requestedMode, requestedCategory, agentContractInput) {
...
function selectAdaptiveHybridProfile(intent, requestedCategory) {
...
function resolveQueryCategories(requestedCategory, intent) {
  ...
  if (intent.mode === "models") {
    return uniqueStrings([requestedCategory ?? "it", "general"]).slice(0, MAX_QUERY_CATEGORIES);
  }
...
function resolveQueryCategoriesV14(intent, requestedCategory, profile) {
  if (requestedCategory === "images" || requestedCategory === "videos" || requestedCategory === "news") {
    return [requestedCategory];
  }
...
function buildRetrievalPlanV14(query, intent, requestedCategory, language) {
  ...
  const addVariant = (fragments, rationale) => {
  ...
  switch (profile.bucket) {
    case "guarded-official-docs":
      addVariant(
        docsTroubleshootingLike ? zhLike ? ["\u6545\u969C\u6392\u67E5", "\u5B89\u88C5", "\u6307\u5357"] : ["troubleshooting", "install", "guide"] : zhLike ? ["\u5B98\u65B9", "\u6587\u6863", "API"] : ["official docs", "api reference"],
        docsTroubleshootingLike ? ["docs-rewrite", "troubleshooting-guide"] : ["docs-rewrite", "official-docs"]
      );
      break;
    case "guarded-github":
...
function decontaminateResultsV14(results, intent, requestedCategory, debug = false, options = {}) {
  ...
  const pageSeekingIntent = intent.docsLike || intent.githubLike || intent.modelLike || intent.packageLike || intent.sourceMentions.length > 0;
  const precisionLandingGuard = Boolean(options.enablePrecisionLandingGuard) && planner.branch === "precision-lookup" && pageSeekingIntent;
  const inspected = results.map((result) => {
    ...
    const technicalHost = isGitHubHost(host) || isModelHost(host) || isPackageHost(host) || looksLikeOfficialDocs(host, pagePath, result.title);
    const landingLike = resultType === "landing" || resultType === "directory" || isShallowPath(pagePath) && resultType !== "repo" && resultType !== "package" && resultType !== "issue-thread";
    const explicitSourceFit = intent.sourceMentions.includes("github") && isGitHubHost(host) || intent.sourceMentions.includes("huggingface") && hostMatches(host, "huggingface.co") || intent.sourceMentions.includes("hf-mirror") && hostMatches(host, "hf-mirror.com") || intent.sourceMentions.includes("modelscope") && (hostMatches(host, "modelscope.cn") || hostMatches(host, "modelscope.com")) || intent.sourceMentions.includes("pypi") && hostMatches(host, "pypi.org") || intent.sourceMentions.includes("npm") && hostMatches(host, "npmjs.com");
    const clearlyRelevant = explicitSourceFit || textIncludesQueryPhrase(result.title, intent.normalizedQuery) || textIncludesQueryPhrase(result.path, intent.normalizedQuery) || entityMatchStrength >= 0.58 || titleCoverage >= 0.72 || pathCoverage >= 0.55 || focusMatchCount >= Math.max(2, Math.min(matchTokens.length, 3));
    return {
      result,
      focusMatchCount,
      broadMatchCount,
  ...
  const strongPageCandidateExists = precisionLandingGuard && inspected.some(
    (item) => !item.landingLike && item.entityMatchStrength >= 0.38 && (isStructuredLookupResultType(item.resultType) || item.titleCoverage >= 0.55 || item.pathCoverage >= 0.34 || item.focusMatchCount >= Math.max(1, Math.min(matchTokens.length, 2)))
  );
  const preserveEntityAlignedDocsLanding = (item) => precisionLandingGuard && intent.mode === "official-docs" && explicitHostLikeQuery && item.landingLike && item.resultType === "official-docs" && item.entityMatchStrength >= 0.16 && (item.focusMatchCount >= 1 || item.titleCoverage >= 0.34 || item.pathCoverage >= 0.22);
  const cleaned = inspected.filter((item) => {
    ...
    if (item.dockerNoise && item.focusMatchCount === 0) {
      ...
      reason = "seo-noise";
    } else if ((profile.bucket === "news-current" || profile.bucket === "finance-amount" || profile.bucket === "local-discovery" || profile.bucket === "media-creator") && item.technicalHost && item.broadMatchCount <= 1) {
      reason = "off-vertical-technical";
    } else if (!intent.dockerLike && isDemotedHost(item.result.host) && planner.branch === "broad-discovery" && item.entityMatchStrength < 0.18 && item.titleCoverage < 0.34 && item.pathCoverage < 0.24) {
      reason = "demoted-host-low-confidence";
    } else if (precisionLandingGuard && strongPageCandidateExists && item.landingLike && !preserveEntityAlignedDocsLanding(item) && !item.explicitSourceFit && !item.clearlyRelevant && (item.technicalHost || item.dockerNoise || isShallowPath(item.result.path)) && item.focusMatchCount <= 1 && (item.resultType === "directory" || item.pathCoverage < 0.34)) {
  ...
  return {
    ...
    summary: {
      ...
      reasonCounts,
      removedSamples: debug ? removedSamples : void 0
    }
  };
}
async function collectSearchCandidates(cfg, params) {
  ...
  if (!isRetrievalFirstRerankVersion(params.rerankVersion)) {
    ...
    return {
      ...
      merged: mergeSearchResults(groups2),
      decontamination: void 0
    };
  }
  const retrievalPlanBase = buildRetrievalPlanV14(params.query, intent, requestedCategory, params.language);
  const retrievalPlan = params.rerankVersion === "v1.5" ? { ...retrievalPlanBase, strategy: "retrieval-first-v1.5" } : params.rerankVersion === "v2.0" ? { ...retrievalPlanBase, strategy: "retrieval-first-v2.0" } : retrievalPlanBase;
  const fetchPlan = [];
...
function mergeSearchResults(groups) {
  ...
  const unresponsive = /* @__PURE__ */ new Set();
...
function pushSignal(signals, label, score) {
  if (score !== 0) {
...
function scoreResult(result, intent) {
  ...
  switch (intent.mode) {
    case "models":
      if (modelHost) add(7, "mode-models-domain");
      ...
      if (hasAny(title + " " + snippet + " " + pagePath, ["8bit", "4bit", "int8", "mlx", "quant", "quantized", "checkpoint", "model card", "safetensors", "onnx"])) {
        add(2, "mode-models-keywords");
      }
      break;
    case "github":
      if (githubHost) add(8, "mode-github-domain");
    ...
    case "packages":
      ...
      if (githubHost) add(2, "mode-packages-github");
      if (engine.includes("pypi") || engine.includes("npm")) add(2, "mode-packages-engine");
      if (engine.includes("docker") && !intent.dockerLike) add(-3.5, "mode-packages-docker");
      break;
    case "official-docs":
      if (docsHost) add(6.5, "mode-docs-domain");
      if (!docsHost && !githubHost) add(-2, "mode-docs-nondoc");
      if (githubHost && pagePath.includes("readme")) add(2, "mode-docs-readme");
      if (intent.officialLike && docsHost) add(1.5, "mode-docs-official");
      break;
    case "general":
      if (docsHost && intent.docsLike) add(2, "general-docs-hint");
      if (githubHost && intent.githubLike) add(2, "general-github-hint");
...
function rerankResultsV11(results, intent, debug = false) {
  ...
  }).sort((a, b) => (b.score ?? 0) - (a.score ?? 0) || a.originalRank - b.originalRank).map((result, index) => ({
    ...result,
    rank: index + 1,
    signals: debug ? result.signals : void 0
  }));
}
function rerankResults(results, intent, limit, debug = false) {
  return ensureWithinLimit(rerankResultsV11(results, intent, debug), limit);
}
function rerankStrategyLabel(version) {
  switch (version) {
    case "v1.0":
...
function resolveRequestedRerankVersion(value) {
  return isSupportedRerankVersion(value) ? value : void 0;
}
function resolveEffectiveRerankVersion(cfg, params) {
...
function compressPriorTowardNeutral(value, strength) {
...
function resolveLocalEmbeddingModelPath(cfg) {
...
async function getLocalEmbeddingProvider(cfg) {
  if (!localEmbeddingProviderPromise) {
    localEmbeddingProviderPromise = (async () => {
      ...
      let initError = null;
      const sanitizeAndNormalizeEmbedding = (vector) => {
...
function snippetTextForEmbedding(result) {
...
function adaptiveTextForEmbedding(result) {
  const title = result.title?.trim();
  const snippet = result.snippet?.trim();
  if (title && snippet) {
...
function minMaxNormalize(values) {
  ...
  const range = Math.max(1e-4, maxValue - minValue);
  return values.map((value) => Number(((value - minValue) / range).toFixed(4)));
}
function hashSemanticFeature(input) {
  let hash = 2166136261;
...
function hashedSemanticVector(text, size = FALLBACK_SEMANTIC_VECTOR_SIZE) {
...
function buildFallbackSemanticEmbeddings(query, texts, reason, providerEntry) {
  return {
...
async function rerankResultsV12(cfg, results, intent, debug = false) {
  ...
  try {
    ...
    const v11ScoreRange = Math.max(1e-4, maxV11Score - minV11Score);
    const reranked = v11.map((result, index) => {
    ...
    }).sort((a, b) => (b.score ?? 0) - (a.score ?? 0) || (b.embeddingSimilarity ?? 0) - (a.embeddingSimilarity ?? 0) || (a.rank ?? 999) - (b.rank ?? 999) || a.originalRank - b.originalRank).map((result, index) => ({
      ...result,
      rank: index + 1,
      signals: debug ? result.signals : void 0
    }));
...
async function rerankResultsV13(cfg, results, intent, requestedCategory, debug = false) {
  const v11 = rerankResultsV11(results, intent, true);
  const providerEntry = await getLocalEmbeddingProvider(cfg);
  const profile = selectAdaptiveHybridProfile(intent, requestedCategory);
  ...
  try {
    ...
    const reranked = v11.map((result, index) => {
      ...
      (a, b) => (b.score ?? 0) - (a.score ?? 0) || (guardedProfile ? (b.heuristicPrior ?? 0) - (a.heuristicPrior ?? 0) : (b.semanticScore ?? 0) - (a.semanticScore ?? 0)) || (a.rank ?? 999) - (b.rank ?? 999) || a.originalRank - b.originalRank
    ).map((result, index) => ({
      ...result,
      rank: index + 1,
      signals: debug ? result.signals : void 0
...
function resultSourceAspectFamilyV15(result, mode) {
  const host = normalizeText(result.host);
  if (isGitHubHost(host)) {
    return "github";
  }
  ...
  const docsSurfaceLike = DOC_HOST_HINTS.some((hint) => hostMatches(host, hint)) || hasDocsHostPrefix(host) || DOC_PATH_HINTS.some((hint) => result.path.includes(hint));
  if (docsSurfaceLike) {
    return mode === "official-docs" ? "official-docs" : `docs:${host}`;
  }
  return host;
}
function rebalanceMultiSourceSlateV15(results, limit, debug = false, options = {}) {
  ...
  const selected = [];
  const deferred = [];
  const familyFor = (candidate) => resultSourceAspectFamilyV15(candidate, options.mode ?? "general");
  const hasHost = (candidate) => selected.some((item) => item.host === candidate.host);
  const hasFamily = (candidate) => options.preferDistinctFamily ? selected.some((item) => familyFor(item) === familyFor(candidate)) : false;
  ...
  for (const candidate of results.slice(preserveTopN)) {
    ...
    selected.push({
      ...candidate,
      signals: debug ? [...candidate.signals ?? [], `v1.5-slate:${familyAlreadySelected ? "family-backfill" : hostAlreadySelected ? "host-backfill" : "diverse-pass"}`] : candidate.signals
    });
    if (selected.length >= limit) {
      break;
    }
...
function resultDiagnosticsSnapshotV15(result) {
  return {
...
function resultEvidenceConfidenceV15(result, diagnostics, intent, planner) {
  let confidence = diagnostics.entityMatchStrength * 0.34 + diagnostics.sourceFitScore * 0.3 + diagnostics.pageSpecificity * 0.16 + diagnostics.extractionLikelihood * (planner.flags.extractionImportant ? 0.1 : 0.05) + diagnostics.solutionLikelihood * (planner.branch === "solution-hunt" ? 0.12 : 0.04);
...
function stabilityProfileV15(intent, planner) {
  const guarded = intent.mode !== "general" || planner.branch === "precision-lookup" || planner.flags.exactEntityLikely || planner.flags.solutionIntentLikely;
  return {
    enabled: guarded || planner.branch === "broad-discovery",
    guarded,
    anchorStrongTopN: guarded ? intent.mode === "github" || intent.mode === "packages" || intent.mode === "models" ? 2 : 1 : 0,
...
function canPromoteCandidateOverV15(candidate, previous, intent, planner, profile) {
  ...
  if (profile.guarded && (candidateDiagnostics.resultType === "issue-thread" || candidateDiagnostics.resultType === "repo" || candidateDiagnostics.resultType === "package") && candidateDiagnostics.entityMatchStrength >= previousDiagnostics.entityMatchStrength + 0.12 && candidateDiagnostics.pageSpecificity >= previousDiagnostics.pageSpecificity + 0.08) {
    return true;
  }
  if (profile.guarded && candidateDiagnostics.sourceFitScore + 0.16 < previousDiagnostics.sourceFitScore && scoreDelta < profile.overrideScoreDelta) {
    return false;
  }
  ...
  if (profile.guarded && candidateDiagnostics.resultType === "unknown" && previousDiagnostics.resultType !== "unknown" && confidenceDelta < profile.overrideConfidenceDelta) {
    return false;
  }
  return scoreDelta >= profile.minScoreDelta || confidenceDelta >= profile.minConfidenceDelta;
}
function stabilizeRankedResultsV15(results, intent, planner, limit, debug = false) {
  ...
  const working = results.slice(0, windowSize).sort(
    (a, b) => a.originalRank - b.originalRank || (a.rank ?? 999) - (b.rank ?? 999) || a.originalRank - b.originalRank
  );
...
function shouldApplyMultiSourceSlateV15(intent, planner) {
  return planner.branch === "broad-discovery" || planner.flags.extractionImportant || intent.mode === "models" || intent.mode === "packages";
}
function multiSourceSlateOptionsV15(intent, planner) {
  let preserveTopN = 0;
...
function rerankResultsV15(results, intent, planner, limit, debug = false) {
  ...
  const adjusted = results.map((result) => {
    ...
    const guardedAdjustment = applyGuardedLayer ? guardedAdjustmentForResult(result, diagnostics, intent, planner) : 0;
  ...
  if (!applyGuardedLayer) {
    ...
    const stabilized2 = stabilizeRankedResultsV15(adjusted, intent, planner, limit, debug);
    if (shouldApplyMultiSourceSlateV15(intent, planner)) {
      return rebalanceMultiSourceSlateV15(stabilized2, limit, debug, multiSourceSlateOptionsV15(intent, planner));
    }
    return ensureWithinLimit(stabilized2, limit).map((result, index) => ({
      ...result,
      rank: index + 1,
      signals: debug ? result.signals : void 0
    }));
  ...
  const sorted = adjusted.sort(
    (a, b) => (b.score ?? 0) - (a.score ?? 0) || (b.sourceFitScore ?? 0) - (a.sourceFitScore ?? 0) || (b.pageSpecificity ?? 0) - (a.pageSpecificity ?? 0) || (b.entityMatchStrength ?? 0) - (a.entityMatchStrength ?? 0) || (b.heuristicPrior ?? 0) - (a.heuristicPrior ?? 0) || (a.rank ?? 999) - (b.rank ?? 999) || a.originalRank - b.originalRank
  );
...
function resultDiagnosticsSnapshotV20(result) {
  return {
    resultType: result.resultType ?? "unknown",
    solutionLikelihood: result.solutionLikelihood ?? 0,
    entityMatchStrength: result.entityMatchStrength ?? 0,
...
function hybridControllerProfileV20(intent, planner) {
  const exactComparisonLock = planner.queryProfile.hasComparisonHint && !planner.queryProfile.hasSimilarityHint && !intent.normalizedQuery.includes(" landscape") && precisionAnchorTokensV20(intent).length >= 3;
  const lexicalGuard = planner.branch === "precision-lookup" || planner.branch === "solution-hunt" || planner.flags.exactEntityLikely || planner.queryProfile.hasErrorLikePattern || precisionAnchorTokensV20(intent).length >= 3 || planner.queryProfile.hasComparisonHint && precisionAnchorTokensV20(intent).length >= 2;
  const riskSensitive = planner.flags.verifySensitive && intent.mode === "general";
  const strictPrecision = planner.branch === "precision-lookup" || intent.mode !== "general" || planner.flags.exactEntityLikely || exactComparisonLock;
  ...
  const mode = riskSensitive ? "risk-locked" : strictPrecision ? "precision-locked" : artifactAware ? "artifact-aware" : planner.branch === "broad-discovery" ? "broad" : "balanced";
  return {
    mode,
    strictPrecision,
    artifactAware,
    ...
    comparisonAware,
    riskSensitive,
    anchorStrongTopN: riskSensitive ? 3 : strictPrecision ? intent.mode === "github" || intent.mode === "packages" || intent.mode === "models" ? 2 : 1 : comparisonAware ? 1 : artifactAware ? 1 : 0,
...
function hybridOverlayAdjustmentV20(result, diagnostics, intent, planner, profile) {
  ...
  if (profile.artifactAware && artifactIntent && (diagnostics.pageRole === "official-artifact" || diagnostics.pageRole === "repository") && artifactCoverage >= 0.45) {
    adjustment += 0.14;
  }
  if (profile.artifactAware && artifactIntent && (diagnostics.pageRole === "canonical-doc" || diagnostics.pageRole === "official-artifact" || diagnostics.pageRole === "landing") && artifactCoverage < 0.35 && diagnostics.pageSpecificity < 0.3) {
    adjustment -= 0.22;
  }
  if (profile.artifactAware && artifactIntent && intent.mode === "official-docs" && diagnostics.pageRole === "official-artifact") {
    if (hostCoverage >= 0.25) {
      adjustment += 0.14;
  ...
  if (profile.artifactAware && artifactIntent && intent.mode === "official-docs" && diagnostics.pageRole === "canonical-doc" && artifactCoverage < 0.35) {
  ...
  if (diagnostics.pageRole === "meta-listing" || diagnostics.pageRole === "landing") {
    adjustment -= profile.riskSensitive ? 0.16 : profile.comparisonAware || profile.lexicalGuard && lexicalCoverage >= 0.58 ? 0.04 : 0.12;
  }
  ...
  if (profile.strictPrecision && intent.mode === "official-docs" && diagnostics.resultType === "official-docs" && diagnostics.sourceFitScore < 0.8 && diagnostics.entityMatchStrength < 0.36) {
    adjustment -= 0.18;
  }
  if (profile.strictPrecision && intent.mode === "official-docs" && planner.flags.solutionIntentLikely && diagnostics.pageRole === "canonical-doc" && diagnostics.sourceFitScore >= 0.5) {
    adjustment += hasTroubleshootingCueV20(result) ? 0.34 : 0.16;
  }
  if (profile.strictPrecision && intent.mode === "official-docs" && planner.flags.solutionIntentLikely && diagnostics.pageRole === "discussion" && diagnostics.sourceFitScore < 0.3 && !hasTroubleshootingCueV20(result)) {
    adjustment -= 0.08;
  }
  if (profile.strictPrecision && diagnostics.pageRole === "deep-content" && diagnostics.sourceFitScore < 0.55) {
    adjustment -= 0.08;
  }
  if (explainerIntent && (planner.branch === "solution-hunt" || planner.branch === "broad-discovery") && (diagnostics.pageRole === "deep-content" || diagnostics.pageRole === "canonical-doc")) {
  ...
  if (agentArtifactPriority && intent.mode === "official-docs" && (diagnostics.pageRole === "canonical-doc" || diagnostics.pageRole === "official-artifact") && diagnostics.sourceFitScore >= 0.82) {
    adjustment += diagnostics.pageRole === "canonical-doc" ? 0.18 : 0.22;
  }
  if (agentArtifactPriority && intent.mode === "official-docs" && diagnostics.pageRole === "repository" && isGitHubHost(result.host) && (artifactCoverage >= 0.34 || diagnostics.sourceFitScore >= 0.4)) {
    adjustment += 0.16;
  }
  if (agentArtifactPriority && (diagnostics.pageRole === "landing" || diagnostics.pageRole === "deep-content" || diagnostics.pageRole === "meta-listing") && !isGitHubHost(result.host) && diagnostics.sourceFitScore < 0.5 && hostCoverage < 0.25) {
    adjustment -= 0.18;
  }
  if (hasAgentComparePriorityV20(intent) && profile.comparisonAware && lexicalCoverage >= 0.5) {
    adjustment += 0.04;
  }
...
function resultEvidenceConfidenceV20(result, diagnostics, intent, planner, profile) {
  const hostCoverage = resultHostCoverageV20(result, intent);
  const lexicalCoverage = resultLexicalAnchorCoverageV20(result, intent);
  const criticalCoverage = resultCriticalAnchorCoverageV20(result, intent);
  const artifactCoverage = resultArtifactAnchorCoverageV20(result, intent);
  ...
  const explainerIntent = hasExplainerIntentV20(intent, planner);
  const explainerAspectCoverage = resultExplainerAspectCoverageV20(result, intent);
  let confidence = diagnostics.sourceFitScore * (profile.riskSensitive ? 0.28 : 0.34) + diagnostics.entityMatchStrength * (profile.riskSensitive ? 0.24 : 0.28) + lexicalCoverage * (profile.riskSensitive ? 0.42 : profile.lexicalGuard ? 0.34 : 0.1) + diagnostics.pageSpecificity * (profile.riskSensitive ? 0.12 : 0.16) + diagnostics.extractionLikelihood * (profile.artifactAware ? 0.14 : 0.06) + diagnostics.solutionLikelihood * (profile.riskSensitive ? 0.01 : planner.branch === "solution-hunt" ? 0.1 : 0.03) + artifactCoverage * (profile.artifactAware ? 0.08 : 0.02);
  ...
  if (diagnostics.pageRole === "meta-listing" || diagnostics.pageRole === "landing") {
    confidence -= profile.riskSensitive ? 0.14 : profile.comparisonAware || profile.lexicalGuard && lexicalCoverage >= 0.58 ? 0.02 : 0.08;
  }
  if (profile.comparisonAware && diagnostics.pageRole === "unknown") {
    confidence -= 0.08;
  }
...
function canPromoteCandidateOverV20(candidate, previous, intent, planner, profile) {
  ...
  if (profile.riskSensitive && previousDiagnostics.resultType === "news" && candidateDiagnostics.resultType !== "news" && previousBaselineRank <= profile.anchorStrongTopN) {
    return false;
  }
  if (profile.riskSensitive && previousDiagnostics.resultType === "news" && candidateDiagnostics.resultType === "news" && previousBaselineRank <= profile.anchorStrongTopN && candidateCriticalCoverage < previousCriticalCoverage + 0.08 && !(hasAnalysisCommentaryCueV20(previous) && !hasAnalysisCommentaryCueV20(candidate))) {
    if (previousCriticalCoverage < 0.5 && candidateCriticalCoverage < 0.5 && candidateLexicalCoverage < previousLexicalCoverage + 0.22) {
      return false;
    }
    return scoreDelta >= profile.overrideScoreDelta + 0.06 && confidenceDelta >= profile.overrideConfidenceDelta + 0.06 && candidateLexicalCoverage >= previousLexicalCoverage + 0.1;
  }
  if (!profile.strictPrecision && !profile.riskSensitive && previousBaselineRank <= protectedBaselineWindow && candidateConfidence < 0.62 && confidenceDelta < 0.08 && candidateLexicalCoverage <= previousLexicalCoverage + 0.06) {
    return false;
  }
  if (profile.artifactAware && previousBaselineRank <= protectedBaselineWindow && previousArtifactCoverage >= 0.45 && candidateArtifactCoverage + 0.14 < previousArtifactCoverage && previousConfidence >= candidateConfidence - 0.04) {
    return false;
  }
  if (profile.strictPrecision && intent.mode === "github" && previousDiagnostics.pageRole === "repository" && previousDiagnostics.sourceFitScore >= 0.9 && candidate.host !== "github.com" && candidateDiagnostics.pageRole !== "repository" && candidateDiagnostics.pageRole !== "discussion" && candidateDiagnostics.resultType !== "repo" && candidateDiagnostics.resultType !== "issue-thread" && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.18 && previousConfidence >= candidateConfidence - 0.06) {
    return false;
  }
  if (profile.strictPrecision && intent.mode === "github" && candidateDiagnostics.pageRole === "repository" && candidateDiagnostics.sourceFitScore >= 0.9 && candidate.host === "github.com" && previous.host !== "github.com" && previousDiagnostics.pageRole !== "repository" && previousDiagnostics.pageRole !== "discussion" && previousDiagnostics.resultType !== "repo" && previousDiagnostics.resultType !== "issue-thread" && candidateDiagnostics.sourceFitScore >= previousDiagnostics.sourceFitScore + 0.18) {
    return true;
  }
  if (profile.strictPrecision && previousBaselineRank <= protectedBaselineWindow && (previousDiagnostics.pageRole === "canonical-doc" || previousDiagnostics.pageRole === "official-artifact") && previousHostCoverage >= candidateHostCoverage + 0.12 && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.14) {
    return false;
  }
  if (profile.strictPrecision && previousBaselineRank <= protectedBaselineWindow && (previousDiagnostics.pageRole === "canonical-doc" || previousDiagnostics.pageRole === "official-artifact") && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.14 && previousArtifactCoverage >= candidateArtifactCoverage - 0.04 && previousLexicalCoverage >= candidateLexicalCoverage - 0.08 && previousConfidence >= candidateConfidence - 0.06) {
    return false;
  }
  if (profile.strictPrecision && previousBaselineRank <= protectedBaselineWindow && (previousDiagnostics.pageRole === "canonical-doc" || previousDiagnostics.pageRole === "official-artifact") && candidateDiagnostics.pageRole !== "canonical-doc" && candidateDiagnostics.pageRole !== "official-artifact" && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.18) {
    return false;
  }
  if (profile.lexicalGuard && candidateLexicalCoverage + 0.18 < previousLexicalCoverage && candidateConfidence < previousConfidence + 0.08) {
    return false;
  }
  if (profile.riskSensitive && previousBaselineRank <= profile.anchorStrongTopN && previousLexicalCoverage >= candidateLexicalCoverage - 0.02 && previousConfidence >= candidateConfidence - 0.04) {
    return scoreDelta >= profile.overrideScoreDelta + 0.04 && confidenceDelta >= profile.overrideConfidenceDelta + 0.04;
  }
  if (profile.lexicalGuard && previousLexicalCoverage >= 0.62 && candidateLexicalCoverage < previousLexicalCoverage - 0.14 && scoreDelta < profile.overrideScoreDelta) {
    return false;
  }
  if (profile.lexicalGuard && isDemotedHost(candidate.host) && !intent.dockerLike && candidateLexicalCoverage < Math.max(0.6, previousLexicalCoverage - 0.04)) {
    return false;
  }
  if (explainerIntent && (planner.branch === "solution-hunt" || planner.branch === "broad-discovery") && (candidateDiagnostics.pageRole === "deep-content" || candidateDiagnostics.pageRole === "canonical-doc") && (previousDiagnostics.pageRole === "deep-content" || previousDiagnostics.pageRole === "canonical-doc")) {
    ...
    if (candidateExplainerCoverage >= previousExplainerCoverage + 0.25 && candidateDiagnostics.pageSpecificity >= previousDiagnostics.pageSpecificity - 0.06 && candidateConfidence >= previousConfidence - 0.08) {
      return true;
    }
    if (previousExplainerCoverage >= candidateExplainerCoverage + 0.25 && previousConfidence >= candidateConfidence - 0.04) {
      return false;
    }
  }
  if (profile.strictPrecision && intent.mode === "official-docs" && planner.flags.solutionIntentLikely && candidateDiagnostics.pageRole === "canonical-doc" && previousDiagnostics.pageRole === "discussion" && hasTroubleshootingCueV20(candidate) && candidateDiagnostics.sourceFitScore >= previousDiagnostics.sourceFitScore + 0.28 && candidateDiagnostics.solutionLikelihood >= previousDiagnostics.solutionLikelihood - 0.12 && candidateConfidence >= previousConfidence - 0.08) {
    return true;
  }
  if (profile.strictPrecision && intent.mode === "official-docs" && planner.flags.solutionIntentLikely && previousDiagnostics.pageRole === "canonical-doc" && candidateDiagnostics.pageRole === "discussion" && hasTroubleshootingCueV20(previous) && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.28 && previousDiagnostics.solutionLikelihood >= candidateDiagnostics.solutionLikelihood - 0.12) {
    return false;
  }
  if (profile.artifactAware && artifactIntent && intent.mode === "official-docs" && previousDiagnostics.pageRole === "official-artifact" && previousHostCoverage >= 0.25 && candidateDiagnostics.pageRole === "official-artifact" && candidateHostCoverage < 0.25 && !isGitHubHost(candidate.host) && previousArtifactCoverage >= candidateArtifactCoverage - 0.08 && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.12) {
    return false;
  }
  if (profile.artifactAware && artifactIntent && intent.mode === "official-docs" && candidateDiagnostics.pageRole === "official-artifact" && previousDiagnostics.pageRole === "official-artifact" && candidateHostCoverage >= 0.25 && previousHostCoverage < 0.25 && candidateArtifactCoverage >= previousArtifactCoverage - 0.08 && candidateDiagnostics.sourceFitScore >= previousDiagnostics.sourceFitScore + 0.18 && candidateConfidence >= previousConfidence - 0.04) {
    return true;
  }
  if (profile.artifactAware && artifactIntent && intent.mode === "official-docs" && candidateDiagnostics.pageRole === "official-artifact" && previousDiagnostics.pageRole === "canonical-doc" && previousArtifactCoverage < 0.35 && looksLikeVersionedDocsSnapshotV20(previous) && candidateArtifactCoverage >= previousArtifactCoverage + 0.45 && candidateConfidence >= previousConfidence - 0.12) {
    return true;
  }
  if (profile.artifactAware && candidateDiagnostics.pageRole === "official-artifact" && previousDiagnostics.pageRole !== "official-artifact" && candidateDiagnostics.extractionLikelihood >= previousDiagnostics.extractionLikelihood + 0.08 && candidateDiagnostics.sourceFitScore >= previousDiagnostics.sourceFitScore - 0.06 && candidateConfidence >= previousConfidence + 0.02) {
    return true;
  }
  if (profile.artifactAware && candidateDiagnostics.pageRole === "official-artifact" && candidateHostCoverage >= 0.25 && candidateDiagnostics.sourceFitScore >= previousDiagnostics.sourceFitScore + 0.18 && candidateConfidence >= previousConfidence - 0.02) {
    return true;
  }
  if (profile.strictPrecision && (previousDiagnostics.pageRole === "canonical-doc" || previousDiagnostics.pageRole === "repository" || previousDiagnostics.pageRole === "registry" || previousDiagnostics.pageRole === "discussion") && previousDiagnostics.sourceFitScore >= candidateDiagnostics.sourceFitScore + 0.08 && previousConfidence >= candidateConfidence - 0.02 && previousLexicalCoverage >= candidateLexicalCoverage - 0.04) {
    return false;
  }
  if (profile.strictPrecision && (candidateDiagnostics.pageRole === "meta-listing" || candidateDiagnostics.pageRole === "landing") && previousDiagnostics.pageRole !== "meta-listing" && previousDiagnostics.pageRole !== "landing") {
    return false;
  }
  if (previousBaselineRank <= profile.anchorStrongTopN && previousConfidence >= profile.anchoredConfidence) {
    return scoreDelta >= profile.overrideScoreDelta && confidenceDelta >= profile.overrideConfidenceDelta;
...
function isPrecisionLockedArtifactAnchorV20(result, intent) {
...
function stabilizeRankedResultsV20(results, intent, planner, limit, debug = false) {
  ...
  const desired = results.slice(0, windowSize);
  const working = results.slice(0, windowSize).sort(
    (a, b) => (a.rank ?? 999) - (b.rank ?? 999) || a.originalRank - b.originalRank
  );
...
function shouldApplyMultiSourceSlateV20(intent, planner, profile) {
  const guardedDocsBackfill = intent.mode === "official-docs" && profile.strictPrecision && !profile.artifactAware && planner.branch === "precision-lookup";
  if (profile.riskSensitive) {
    return false;
  }
  if (profile.strictPrecision && !profile.artifactAware && intent.mode !== "models" && intent.mode !== "packages" && !guardedDocsBackfill) {
    return false;
  }
  return guardedDocsBackfill || planner.branch === "broad-discovery" || profile.artifactAware || intent.mode === "models" || intent.mode === "packages";
}
function shouldPreserveExactArtifactTop3V20(intent, planner, profile) {
  const latestArtifactCue = hasAny(intent.normalizedQuery, [" latest", " latest version", " newest", " current version"]);
  return intent.mode === "official-docs" && profile.artifactAware && planner.branch === "precision-lookup" && planner.flags.exactEntityLikely && hasArtifactIntentV20(intent) && (latestArtifactCue || hasAgentArtifactPriorityV20(intent));
}
function multiSourceSlateOptionsV20(intent, planner, profile) {
  ...
  return {
    preserveTopN,
    preferDistinctFamily: exactArtifactTop3 || guardedDocsBackfill || profile.artifactAware || planner.branch === "broad-discovery" || intent.mode === "models" || intent.mode === "packages",
    preserveDuplicateFamiliesWithinTopN: exactArtifactTop3 || guardedDocsBackfill || intent.mode === "packages" || intent.mode === "models" && planner.branch === "precision-lookup",
    preserveDuplicateHostsWithinTopN: exactArtifactTop3 || guardedDocsBackfill || intent.mode === "packages" || intent.mode === "models" && planner.branch === "precision-lookup",
    mode: intent.mode
  };
}
function rerankResultsV20(results, intent, planner, limit, debug = false) {
  const profile = hybridControllerProfileV20(intent, planner);
  const adjusted = results.map((result) => {
    const diagnostics = resultDiagnosticsSnapshotV20(result);
...
async function rankMergedSearchResults(cfg, merged, params) {
  ...
  const baseline = ensureWithinLimit(
    merged.results.slice().sort((a, b) => a.originalRank - b.originalRank).map((result, index) => ({
      ...result,
      rank: index + 1
    })),
    params.limit
  ...
  } else if (params.rerankVersion === "v1.3" || params.rerankVersion === "v1.4" || params.rerankVersion === "v1.5" || params.rerankVersion === "v2.0") {
    const v13 = await rerankResultsV13(cfg, merged.results, intent, requestedCategory, debug);
    finalResults = params.rerankVersion === "v2.0" ? rerankResultsV20(
      ...
      params.limit,
      debug
    ) : isPlannerCandidateRerankVersion(params.rerankVersion) ? rerankResultsV15(
      annotateResultDiagnostics(v13.results, intent, planner, {
        applyBranchAdjustment: true,
        applyGuardedAdjustment: false,
        debug
...
async function searchSearxng(cfg, params) {
  ...
  return {
    ...
    retrieval: {
      ...
      categoriesQueried,
      queryVariants: debug ? retrieved.retrievalPlan.variants.map((variant) => ({
        query: variant.query,
    ...
    results: finalResults.map((result, index) => {
      const base = {
        ...
        resultType: debug ? result.resultType : void 0,
        solutionLikelihood: debug ? result.solutionLikelihood : void 0,
        entityMatchStrength: debug ? result.entityMatchStrength : void 0,
        extractionLikelihood: debug ? result.extractionLikelihood : void 0,
        diversityValue: debug ? result.diversityValue : void 0,
        sourceFitScore: debug ? result.sourceFitScore : void 0,
        pageSpecificity: debug ? result.pageSpecificity : void 0,
        pageRole: debug ? result.pageRole : void 0,
        plannerAdjustment: debug ? result.plannerAdjustment : void 0,
        guardedAdjustment: debug ? result.guardedAdjustment : void 0,
        hybridAdjustment: debug ? result.hybridAdjustment : void 0,
        why: debug ? result.why : void 0
      };
      if (!params.citations) {
        return base;
      }
    ...
    baseline: debug ? ranked.baseline.map((result) => ({
      rank: result.rank,
      originalRank: result.originalRank,
      title: result.title,
      url: result.url,
    ...
    embedding: debug ? embeddingInfo : void 0,
...
function renderResearchReport(params) {
  ...
  if (params.retrieval && typeof params.retrieval.strategy === "string") {
    ...
    if (decontamination && typeof decontamination === "object" && typeof decontamination.removedCount === "number") {
      lines.push(`- Candidate decontamination removed: ${decontamination.removedCount}`);
    }
  }
  lines.push("", "## Results", "");
  for (const item of params.results) {
    ...
    const host = item.host ? `host=${String(item.host)}` : void 0;
    const engine = item.engine ? `engine=${String(item.engine)}` : void 0;
    const categories = Array.isArray(item.categories) && item.categories.length > 0 ? `categories=${item.categories.join("/")}` : void 0;
    const score = typeof item.score === "number" ? `score=${item.score}` : void 0;
    const published = item.publishedDate ? `published=${String(item.publishedDate)}` : void 0;
...
function resolvePluginCfg(api) {
  const cfg = api?.pluginConfig ?? {};
  const workspaceDir = resolveWorkspaceDir(api);
  const serviceRoot = typeof cfg.serviceRoot === "string" && cfg.serviceRoot.trim() ? cfg.serviceRoot : path.join(workspaceDir, "services", "agent-searchkit");
  return {
    workspaceDir,
    serviceRoot,
    runsDir: typeof cfg.runsDir === "string" && cfg.runsDir.trim() ? cfg.runsDir : path.join(serviceRoot, "runs"),
    extractRoot: typeof cfg.extractRoot === "string" && cfg.extractRoot.trim() ? cfg.extractRoot : path.join(serviceRoot, "playwright-fallback"),
    ...
    maxTextChars: typeof cfg.maxTextChars === "number" && cfg.maxTextChars > 0 ? cfg.maxTextChars : DEFAULTS.maxTextChars,
    maxLinks: typeof cfg.maxLinks === "number" && cfg.maxLinks > 0 ? cfg.maxLinks : DEFAULTS.maxLinks,
    rerankEnabled: typeof cfg.rerankEnabled === "boolean" ? cfg.rerankEnabled : DEFAULTS.rerankEnabled,
    defaultMode: typeof cfg.defaultMode === "string" ? cfg.defaultMode : DEFAULTS.defaultMode,
    defaultRerankVersion: isSupportedRerankVersion(cfg.defaultRerankVersion) ? cfg.defaultRerankVersion : DEFAULTS.defaultRerankVersion,
    embeddingModelPath: typeof cfg.embeddingModelPath === "string" && cfg.embeddingModelPath.trim() ? cfg.embeddingModelPath : DEFAULT_LOCAL_EMBEDDING_MODEL_PATH
  };
}
...
async function basicFetchExtract(cfg, url) {
...
function extractLinksFromHtml(html, maxLinks) {
  const links = [];
...
async function browserExtract(cfg, url, screenshot) {
  const scriptPath = path.join(cfg.extractRoot, "scripts", "extract.mjs");
  await fs.access(scriptPath);
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-agent-searchkit-"));
  const outJson = path.join(tmpDir, "page.json");
  const outShot = screenshot ? path.join(tmpDir, "page.png") : void 0;
...
async function runAgentSearchkitSearch(cfg, params) {
  return await searchSearxng(cfg, normalizeEnglishSearchToolInput(params));
}
var plugin = {
  id: "agent-searchkit",
  ...
  configSchema: {
    ...
    properties: {
      ...
      maxTextChars: { type: "number", minimum: 1e3, default: DEFAULTS.maxTextChars },
  ...
  register(api) {
    api.registerTool({
      name: "web_searchkit_status",
      description: "Check local Agent Searchkit stack health, artifact paths, and whether legacy Miniflux/Memos containers are still present.",
      parameters: {
        type: "object",
        additionalProperties: false,
        properties: {}
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        ...
        const result = await searchSearxng(cfg, {
          ...
          safeSearch: typeof params.safeSearch === "number" ? params.safeSearch : void 0,
          mode: typeof params.mode === "string" ? params.mode : void 0,
          rerank: typeof params.rerank === "boolean" ? params.rerank : void 0,
          ...
          citations: typeof params.citations === "boolean" ? params.citations : void 0
        });
      ...
      createTool: () => ({
        ...
        execute: async (args) => {
          ...
          return {
            ...
            sources: citations ? (result.results ?? []).map((r) => r.citation?.formatted).filter((item) => typeof item === "string" && item.length > 0) : void 0
          };
        }
      })
    });
