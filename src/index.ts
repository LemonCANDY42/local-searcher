...
function stripTrailingCjkNewsModifiers(input: string) {
  let compact = input;
  let changed = false;
...
function isCjkNewsModifierPhrase(input: string) {
  if (!hasCjkText(input)) {
    return false;
  }
  let rest = input;
...
function shouldUseNativeSearxngRankingForCjk(queryOrIntent: string | SearchIntent) {
...
async function ensureThreadBindingsExportModule() {
  ...
  const exportDir = path.join(os.tmpdir(), "openclaw-agent-searchkit-plugin-sdk");
  ...
  try {
    ...
    if (!shouldWrite) {
      ...
      shouldWrite = !existing.includes("agent-searchkit-cpu-embed");
    }
  } catch {
    shouldWrite = true;
  }
...
async function searchSearxng(cfg: any, params: {
  ...
  const language = typeof params.language === "string" && params.language.trim()
    ? params.language
    : cfg.defaultLanguage;
  const searchLanguage = normalizeSearchLanguageForQuery(language, params.query);
  const safeSearch =
...
function resolvePluginCfg(api: any) {
  ...
  const serviceRoot =
    typeof cfg.serviceRoot === "string" && cfg.serviceRoot.trim()
      ? cfg.serviceRoot
      : path.join(workspaceDir, "services", "agent-searchkit");
  return {
    workspaceDir,
    serviceRoot,
    runsDir:
...
const plugin = {
  ...
  register(api: any) {
    ...
    api.registerWebSearchProvider({
      ...
      setCredentialValue: () => {},
      getConfiguredCredentialValue: (config?: any) => config?.plugins?.entries?.["agent-searchkit"]?.config?.searxngBaseUrl,
      setConfiguredCredentialValue: (configTarget: any, value: unknown) => {
